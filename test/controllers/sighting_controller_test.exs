defmodule Birdie.SightingControllerTest do
  ...
  test "creates resource and redirects when data is valid", %{conn: conn} do
    conn = post conn, admin_sighting_path(conn, :create), sighting: @valid_attrs
    assert redirected_to(conn) == admin_sighting_path(conn, :index)
    assert Repo.get_by(Sighting, @valid_attrs)
  end

  test "does not create resource and renders errors when data is invalid", %{conn: conn} do
    conn = post conn, admin_sighting_path(conn, :create), sighting: @invalid_attrs
    assert html_response(conn, 200) =~ "New sighting"
  end
  ...
  test "updates chosen resource and redirects when data is valid", %{conn: conn} do
    sighting = Repo.insert! %Sighting{}
    conn = put conn, admin_sighting_path(conn, :update, sighting), sighting: @valid_attrs
  ...
  test "does not update chosen resource and renders errors when data is invalid", %{conn: conn} do
    sighting = Repo.insert! %Sighting{}
    conn = put conn, admin_sighting_path(conn, :update, sighting), sighting: @invalid_attrs
    assert html_response(conn, 200) =~ "Edit sighting"
  end
